...
void imprimeMatriz(double ** M, int ordem, FILE * p);

double CalculaDeterminante(double ** a, int n)
{
    int i,j,j1,j2;                
	double det=0;
	double **m = NULL;
    ...
    else
    {
		...
		for (j1=0;j1<n;j1++) 
		{
			...
			det+=pow(-1.0,1.0+j1+1.0)*a[0][j1]*CalculaDeterminante(m,n-1);
...
double ** matrizM(double ** a, double** b, int ordem)
{
	double **c=NULL;
	int i=0,j=0,k=0;
...
void* determinanteMatriz(void* arg)
{
	S * item;
	int ordem;
	while(1)
...
void* escreveArquivo(void* arg)
{
	...
	while(1)
	{
		...
		*item=shared[3].buffer[shared[3].out];
		...
		fprintf(arquivoMat, "---------------------------\n");
		imprimeMatriz(A, ordem, arquivoMat);
		fprintf(arquivoMat, "---------------------------\n");
		imprimeMatriz(B, ordem, arquivoMat);
		fprintf(arquivoMat, "---------------------------\n");
		imprimeMatriz(C, ordem, arquivoMat);
		fprintf(arquivoMat, "---------------------------\n");
...
int main(int argc, char**argv)
{
	...
	for(i=0;i<LC;i++)
		pthread_create(&idLC, NULL, leituraDados, str);
	for(i=0;i<LA;i++)
		pthread_create(&idLA[i], NULL, leituraArquivo, NULL);
	for(i=0;i<MM;i++)
		pthread_create(&idMM[i], NULL, multiplicaMatriz, NULL);
	for(i= 0;i<DM;i++)
		pthread_create(&idDM[i], NULL, determinanteMatriz, NULL);
	for(i= 0;i<EA;i++)
		pthread_create(&idEA[i], NULL, escreveArquivo, &i);
	for(i=0;i<EA;i++)
